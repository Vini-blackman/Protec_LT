...
// Lucide Icons (disponíveis globalmente via CDN)
const { 
  Settings, Zap, Calculator, CheckCircle, FileText, AlertTriangle, 
  ChevronRight, Upload, Download, Activity, Shield, 
  Users, Database, Plus, Trash2, Save, X, Cpu,
  ChevronDown, Info, Check, Copy, Eye, EyeOff, RefreshCw, Search,
  Building2, User, FileSpreadsheet, Image, Printer, Mail,
  Phone, MapPin, Calendar, Clock, Hash, Percent, ArrowRight, ArrowLeft,
  RotateCcw, Maximize2, ZoomIn, ZoomOut, Move, Layers, Grid, Target,
  Circle, Square, Edit, Bookmark, Star, TrendingUp, TrendingDown,
  Table, List, Menu, Filter, Columns, Layout, ExternalLink, Link, File, Folder,
  BarChart2, Edit3
} = lucideReact;

// Aliases para compatibilidade
const BarChart3 = BarChart2 || Activity;
const Edit2 = Edit3 || Edit;
...
const calculateProtectionSettings = (inputs) => {
  ...
  const calculateTerminal = (term, termName) => {
    ...
    // Z3: Backup remoto forward - pode ser % da linha+adjacente ou só linha
    let z3_reach = 0;
    const z3_adj_available = Z_total_mod > Z1_sec_mod * 1.01;
    if (zonas.z3_habilitada) {
    ...
    // Z4: Reversa para backup de barra - usa porcentagem configurada pelo usuário
    const z4_reach = (zonas.z4_percent / 100) * Z1_sec_mod;
    ...
    switch (norma) {
      case 'ONS':
        criteria = { 
          z1: `${zonas.z1_percent}%`, 
          z2: `${zonas.z2_percent}%`, 
          z3: zonas.z3_habilitada ? `${zonas.z3_percent}% ${z3_adj_available ? '(L+Adj)' : '(L)'}` : 'Desabilitada', 
          z4: `${zonas.z4_percent}% Rev`,
      ...
      case 'COES':
        criteria = { 
          ...
          texto_z3: zonas.z3_habilitada 
            ? `A Zona 3 foi calculada como ${zonas.z3_percent}% da ${z3_adj_available ? 'soma vetorial da linha protegida com a linha adjacente' : 'impedância da linha'}, conforme procedimento PR-20.`
            : 'Zona 3 desabilitada conforme configuração do usuário.',
...
function SEPApp() {
  ...
  const [zonasProtecao, setZonasProtecao] = useState({
    ...
    z3_percent: 0,       // Zona 3 - Backup Remoto Forward (0 = não habilitado)
    z4_percent: 20,      // Zona 4 - Reversa / Backup de Barra
    z1_tempo: 0,         // Tempo Z1 (ms) - instantâneo
    z2_tempo: 400,       // Tempo Z2 (ms)
    z3_tempo: 1200,      // Tempo Z3 (ms)
    z4_tempo: 1000,      // Tempo Z4 (ms)
    z3_habilitada: false // Z3 pode ser desabilitada
  });
  ...
  const generateFullReport = () => {
    ...
    const t = {
      ...
      zona3_texto_normal: {
        'pt-BR': `Ajustada em **${zonasProtecao.z3_percent}%** ${zonasProtecao.z3_percent > 100 ? 'da impedância total (linha + adjacente)' : 'da impedância da linha'} para fornecer proteção de retaguarda remota às linhas adjacentes (tempo: ${zonasProtecao.z3_tempo}ms).${!zonasProtecao.z3_habilitada ? ' **NOTA:** Zona 3 está DESABILITADA na configuração.' : ''}`,
        'es-ES': `Ajustada al **${zonasProtecao.z3_percent}%** ${zonasProtecao.z3_percent > 100 ? 'de la impedancia total (línea + adyacente)' : 'de la impedancia de la línea'} para proporcionar protección de respaldo remota a las líneas adyacentes (tiempo: ${zonasProtecao.z3_tempo}ms).${!zonasProtecao.z3_habilitada ? ' **NOTA:** Zona 3 está DESHABILITADA en la configuración.' : ''}`,
        'en-US': `Set at **${zonasProtecao.z3_percent}%** ${zonasProtecao.z3_percent > 100 ? 'of total impedance (line + adjacent)' : 'of line impedance'} to provide remote backup protection for adjacent lines (time: ${zonasProtecao.z3_tempo}ms).${!zonasProtecao.z3_habilitada ? ' **NOTE:** Zone 3 is DISABLED in configuration.' : ''}`
      },
      k0_texto: {
      ...
      // Seção 8 - Conclusões
      conclusoes_texto: {
        'pt-BR': `Com base nos cálculos apresentados, conclui-se que os ajustes propostos atendem aos requisitos de sensibilidade, seletividade e rapidez exigidos pela norma ${projeto.norma}. Os alcances de proteção foram definidos em: **Zona 1: ${zonasProtecao.z1_percent}%** (instantânea), **Zona 2: ${zonasProtecao.z2_percent}%** (temporizada ${zonasProtecao.z2_tempo}ms)${zonasProtecao.z3_habilitada ? `, **Zona 3: ${zonasProtecao.z3_percent}%** (retaguarda ${zonasProtecao.z3_tempo}ms)` : ''} e **Zona 4: ${zonasProtecao.z4_percent}%** (reversa).${zonasProtecao.z1_percent > 85 ? ' **ATENÇÃO:** O alcance de Zona 1 excede a recomendação típica de 85%, assumindo-se risco controlado de sobrealcance transitório.' : ''}${zonasProtecao.z1_percent < 75 ? ' **NOTA:** O alcance de Zona 1 está abaixo do típico, priorizando segurança sobre cobertura instantânea.' : ''} A verificação de carga (Loadability) confirma que não haverá atuação indevida para o fluxo de potência máximo previsto. O sistema de proteção está apto para operação segura.`,
        'es-ES': `Con base en los cálculos presentados, se concluye que los ajustes propuestos cumplen con los requisitos de sensibilidad, selectividad y rapidez exigidos por la norma ${projeto.norma}. Los alcances de protección fueron definidos en: **Zona 1: ${zonasProtecao.z1_percent}%** (instantánea), **Zona 2: ${zonasProtecao.z2_percent}%** (temporizada ${zonasProtecao.z2_tempo}ms)${zonasProtecao.z3_habilitada ? `, **Zona 3: ${zonasProtecao.z3_percent}%** (respaldo ${zonasProtecao.z3_tempo}ms)` : ''} y **Zona 4: ${zonasProtecao.z4_percent}%** (reversa).${zonasProtecao.z1_percent > 85 ? ' **ATENCIÓN:** El alcance de Zona 1 excede la recomendación típica de 85%, asumiendo riesgo controlado de sobrealcance transitorio.' : ''}${zonasProtecao.z1_percent < 75 ? ' **NOTA:** El alcance de Zona 1 está por debajo del típico, priorizando seguridad sobre cobertura instantánea.' : ''} La verificación de carga (Loadability) confirma que no habrá actuación indebida para el flujo de potencia máximo previsto. El sistema de protección está apto para operación segura.`,
        'en-US': `Based on the calculations presented, it is concluded that the proposed settings meet the sensitivity, selectivity, and speed requirements of the ${projeto.norma} standard. Protection reaches were defined as: **Zone 1: ${zonasProtecao.z1_percent}%** (instantaneous), **Zone 2: ${zonasProtecao.z2_percent}%** (timed ${zonasProtecao.z2_tempo}ms)${zonasProtecao.z3_habilitada ? `, **Zone 3: ${zonasProtecao.z3_percent}%** (backup ${zonasProtecao.z3_tempo}ms)` : ''} and **Zone 4: ${zonasProtecao.z4_percent}%** (reverse).${zonasProtecao.z1_percent > 85 ? ' **WARNING:** Zone 1 reach exceeds typical 85% recommendation, assuming controlled transient overreach risk.' : ''}${zonasProtecao.z1_percent < 75 ? ' **NOTE:** Zone 1 reach is below typical, prioritizing safety over instantaneous coverage.' : ''} Load verification (Loadability) confirms there will be no undue operation for the maximum expected power flow. The protection system is ready for safe operation.`
      },
  ...
  const exportCSV = () => {
    ...
    csv += `Z3 Alcance,${termA.z3_reach},${termB.z3_reach},Ω,${zonasProtecao.z3_habilitada ? zonasProtecao.z3_percent + '% (L+Adj)' : 'Desabilitada'}\n`;
      ...
      <div class="page">
        ...
        <table style="font-size: 9pt;">
          ...
          <tr>
            ...
            <td>Ω (sec)</td>
            <td>${zonasProtecao.z3_percent}% (Retaguarda)</td>
          </tr>
          <tr>
          ...
          <tr>
            <td><strong>Z3MAG</strong></td>
            <td>Alcance Zona 3</td>
            <td colspan="2" style="text-align: center; color: #999;">DESABILITADA</td>
            <td>Ω (sec)</td>
            <td>Zona 3 não habilitada neste estudo</td>
          </tr>
          `}
          <tr>
            <td><strong>Z4MAG</strong></td>
            <td>Alcance Zona 4 (Reversa)</td>
            ...
            <td>Ω (sec)</td>
            <td>${zonasProtecao.z4_percent}% (Direção: REVERSE)</td>
          </tr>
      ...
      <div class="page" style="page-break-before: always;">
        ...
        <div style="background: #e8f4fd; border: 1px solid #b6d4fe; border-radius: 6px; padding: 12px; margin-top: 16px;">
          ...
          <ul style="margin: 0; padding-left: 20px; font-size: 10pt; color: #333;">
            ...
            <li><strong>Zonas Configuradas:</strong> Z1=${zonasProtecao.z1_percent}% | Z2=${zonasProtecao.z2_percent}% | Z3=${zonasProtecao.z3_habilitada ? zonasProtecao.z3_percent + '%' : 'OFF'} | Z4=${zonasProtecao.z4_percent}% (Rev)</li>
          ...
          <>
            ...
            <div className="panel">
              ...
              <div style={{ padding: '20px', display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '20px' }}>
                ...
                <div style={{ 
                  background: zonasProtecao.z3_habilitada ? 'rgba(249,115,22,0.08)' : 'rgba(128,128,128,0.05)', 
                  border: `1px solid ${zonasProtecao.z3_habilitada ? 'rgba(249,115,22,0.3)' : 'rgba(128,128,128,0.2)'}`, 
                  borderRadius: '12px', 
                  padding: '20px',
                  position: 'relative',
                  opacity: zonasProtecao.z3_habilitada ? 1 : 0.7
                }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '16px' }}>
                    <div style={{ 
                      width: '36px', height: '36px', 
                      background: zonasProtecao.z3_habilitada 
                    ...
                    <div>
                      <h4 style={{ color: zonasProtecao.z3_habilitada ? '#f97316' : '#6b7280', margin: 0, fontSize: '15px' }}>
                        Zona 3 - Backup Remoto (Forward)
                      </h4>
                    ...
                    <label style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer' }}>
                      <input
                        type="checkbox"
                        checked={zonasProtecao.z3_habilitada}
                        onChange={(e) => setZonasProtecao({...zonasProtecao, z3_habilitada: e.target.checked})}
                    ...
                    <div className="input-group">
                      ...
                      <div style={{ position: 'relative' }}>
                        <input
                          ...
                          disabled={!zonasProtecao.z3_habilitada}
                          style={{ 
                            width: '100%', padding: '10px 40px 10px 14px', 
                            background: '#161b22', border: '1px solid #30363d', 
                            borderRadius: '8px', color: zonasProtecao.z3_habilitada ? '#e6edf3' : '#6b7280', 
                  ...
                  <div style={{ 
                    marginTop: '12px', padding: '10px 12px', 
                    background: zonasProtecao.z3_habilitada ? 'rgba(249,115,22,0.1)' : 'rgba(128,128,128,0.05)', 
                    borderRadius: '6px',
                    fontSize: '11px', color: zonasProtecao.z3_habilitada ? '#fdba74' : '#9ca3af'
                  }}>
                ...
                <div style={{ 
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '16px' }}>
                    ...
                    <div>
                      <h4 style={{ color: '#a855f7', margin: 0, fontSize: '15px' }}>Zona 4 - Reversa</h4>
                      <span style={{ fontSize: '11px', color: '#8b949e' }}>Backup de Barra • Falha de Disjuntor</span>
                    </div>
                    <div 
                      style={{ marginLeft: 'auto', cursor: 'help' }}
                      title="Zona 4: Proteção reversa para backup de falhas no barramento local ou esquema de falha de disjuntor (STUB). Direção oposta às demais zonas."
                    >
                      <AlertTriangle size={16} color="#8b949e" />
                    </div>
                  </div>
                  ...
                  <div style={{ 
                    ...
                    <strong>📋 Recomendação COES:</strong> 20% Reverso (mínimo 10-25%)
                    <br/>
                    <span style={{ color: '#8b949e' }}>💡 Proteção de falha de disjuntor ou barras. Direção: REVERSA (atrás do relé).</span>
                    <br/>
              ...
              <div style={{ 
                ...
                <div style={{ textAlign: 'center' }}>
                  <div style={{ fontSize: '10px', color: zonasProtecao.z3_habilitada ? '#f97316' : '#6b7280', marginBottom: '4px' }}>Z3 Ajuste</div>
                  <div style={{ fontSize: '18px', fontWeight: '700', color: zonasProtecao.z3_habilitada ? '#f97316' : '#6b7280', fontFamily: 'JetBrains Mono' }}>
                    {zonasProtecao.z3_habilitada ? ((Math.sqrt(linha.R1**2 + linha.X1**2) * projeto.linha_comprimento) * zonasProtecao.z3_percent / 100).toFixed(2) + ' Ω' : 'N/A'}
                  </div>
                  <div style={{ fontSize: '10px', color: '#8b949e' }}>{zonasProtecao.z3_habilitada ? `${zonasProtecao.z3_percent}% • ${zonasProtecao.z3_tempo}ms` : 'Desabilitada'}</div>
                </div>
                <div style={{ textAlign: 'center' }}>
                  <div style={{ fontSize: '10px', color: '#a855f7', marginBottom: '4px' }}>Z4 Ajuste (Rev)</div>
                  <div style={{ fontSize: '18px', fontWeight: '700', color: '#a855f7', fontFamily: 'JetBrains Mono' }}>
                    {((Math.sqrt(linha.R1**2 + linha.X1**2) * projeto.linha_comprimento) * zonasProtecao.z4_percent / 100).toFixed(2)} Ω
                  </div>
                  <div style={{ fontSize: '10px', color: '#8b949e' }}>{zonasProtecao.z4_percent}% • {zonasProtecao.z4_tempo}ms</div>
                </div>
              </div>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px' }}>
                        ...
                        <div style={{ textAlign: 'center' }}>
                          ...
                          <div style={{ fontSize: '16px', fontWeight: '700', color: '#f97316', fontFamily: 'JetBrains Mono' }}>
                            {zonasProtecao.z3_habilitada ? (previewCalc.terminal_A?.ansi_21?.z3_reach?.toFixed(2) || '—') : 'N/A'}
                          </div>
                          <div style={{ fontSize: '9px', color: '#6e7681' }}>Ω</div>
                        </div>
                        <div style={{ textAlign: 'center' }}>
                    ...
                    <div style={{ 
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px' }}>
                        ...
                        <div style={{ textAlign: 'center' }}>
                          <div style={{ fontSize: '10px', color: '#8b949e' }}>Z3 (Sec)</div>
                          <div style={{ fontSize: '16px', fontWeight: '700', color: '#f97316', fontFamily: 'JetBrains Mono' }}>
                            {zonasProtecao.z3_habilitada ? (previewCalc.terminal_B?.ansi_21?.z3_reach?.toFixed(2) || '—') : 'N/A'}
                          </div>
                          <div style={{ fontSize: '9px', color: '#6e7681' }}>Ω</div>
                        </div>
                        <div style={{ textAlign: 'center' }}>
              ...
              <>
                <div className="panel">
                  ...
                  <div className="comparison-table">
                    ...
                    <div className="comparison-label"><span className="zone-chip z4">Z4</span> Reversa ({results.terminal_A.criteria.z4})</div>
                    <div className="comparison-value local">{results.terminal_A.z4_reach} Ω</div>
                    <div className="comparison-value remote">{results.terminal_B.z4_reach} Ω</div>
                ...
                <div className="card-content" style={{ padding: '0' }}>
                  {results ? (
                    (() => {
                      ...
                      const mhoZ4A = generateMhoCircle(z4Sec, angLinha, 0, 0, true); // REVERSA!
                      ...
                      if (coordConfig.showZ3 && zonasProtecao.z3_habilitada) allBounds.push(getAllBounds(mhoZ3A));
                      if (coordConfig.showZ4) allBounds.push(getAllBounds(mhoZ4A)); // Zona 4 reversa
                      ...
                      return (
                        ...
                        >
                          ...
                          {/* Z3 - Retaguarda (laranja, tracejado) */}
                          {coordConfig.showZ3 && zonasProtecao.z3_habilitada && pathZ3A && (
                            <g>
                          ...
                          {/* Z4 - Reversa (roxo, tracejado) */}
                          {coordConfig.showZ4 && pathZ4A && (
                            <g>
                          ...
                          {coordConfig.showLabels && (
                            <>
                              ...
                              {coordConfig.showZ3 && zonasProtecao.z3_habilitada && (() => {
                              ...
                              {coordConfig.showZ4 && (() => {
                                const pos = getLabelPosition(z4Sec, angLinha, 0, 0, true);
                                return (
                                  <g>
                                    <rect x={pos.px - 35} y={pos.py - 10} width={70} height={18} fill="#161b22" stroke="#a855f7" strokeWidth={1} rx={4} opacity={0.95} />
                                    <text x={pos.px} y={pos.py + 4} fill="#a855f7" fontSize="11" fontWeight="bold" textAnchor="middle">Z4 (Rev)</text>
                                  </g>
                                );
                              })()}
                            </>
                          ...
                          <g transform="translate(12, 70)">
                            ...
                            <text x={48} y={126} fill="#a855f7" fontSize="10">Z4: {z4Sec.toFixed(2)} Ω ({zonasProtecao.z4_percent}% Rev)</text>
                          ...
                          {/* Informações de Zona 4 */}
                          {coordConfig.showZ4 && (
                            <g transform={`translate(${svgWidth - 200}, 70)`}>
                              <rect x={0} y={0} width={185} height={80} fill="#0d1117" stroke="#a855f7" rx={8} opacity={0.95} />
                              <text x={92} y={20} fill="#a855f7" fontSize="12" fontWeight="bold" textAnchor="middle">Zona 4 (Reversa)</text>
                              <text x={10} y={40} fill="#8b949e" fontSize="10">Direção: REVERSE</text>
                              <text x={10} y={55} fill="#8b949e" fontSize="10">Alcance: {z4Sec.toFixed(2)} Ω ({zonasProtecao.z4_percent}%)</text>
                              <text x={10} y={70} fill="#8b949e" fontSize="10">Função: Proteção de Barra/BF</text>
                            </g>
                          )}
